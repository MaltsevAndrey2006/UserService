...
public class PaymentCardIntegrationTest {
    ...
    void shouldCreatePaymentCard() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/v1/payment-cards")
                        .header("Authorization", "Bearer " + jwtToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.number").value("**** **** **** 2412"))
                .andExpect(jsonPath("$.holder").value("**** **** **** drey"))
                .andExpect(jsonPath("$.active").value(true))
                .andExpect(jsonPath("$.userId").value(testUser.getId()))
                .andReturn();

        PaymentCardResponse response = objectMapper.readValue(result.getResponse().getContentAsString(), PaymentCardResponse.class);
        assertThat(response.getId()).isNotNull();
        assertThat(response.getNumber()).isEqualTo("**** **** **** 2412");
        assertThat(response.getHolder()).isEqualTo("**** **** **** drey");
    ...
    void shouldReturnPaymentCardById() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/api/v1/payment-cards/{id}", savedCard.getId())
                        .header("Authorization", "Bearer " + jwtToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(savedCard.getId()))
                .andExpect(jsonPath("$.number").value("**** **** **** 3123"))
                .andExpect(jsonPath("$.active").value(true))
                .andExpect(jsonPath("$.holder").value("**** **** **** USER"))
                .andReturn();

        PaymentCardResponse response = objectMapper.readValue(result.getResponse().getContentAsString(), PaymentCardResponse.class);
        assertThat(response.getId()).isEqualTo(savedCard.getId());
        assertThat(response.getNumber()).isEqualTo("**** **** **** 3123");
        assertThat(response.getHolder()).isEqualTo("**** **** **** USER");
    }
    ...
    void shouldReturnCardsByUserId() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/api/v1/payment-cards/user/{userId}", testUser.getId())
                        .header("Authorization", "Bearer " + jwtToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(2))
                .andExpect(jsonPath("$[0].number").value("**** **** **** 3123"))
                .andExpect(jsonPath("$[1].number").value("**** **** **** 2356"))
                .andReturn();

        PaymentCardResponse[] responses = objectMapper.readValue(result.getResponse().getContentAsString(), PaymentCardResponse[].class);
        assertThat(responses).hasSize(2);
        assertThat(responses[0].getNumber()).isEqualTo("**** **** **** 3123");
        assertThat(responses[1].getNumber()).isEqualTo("**** **** **** 2356");
    }
    ...
    @Test
    void shouldNotCreateMoreThan5Cards() throws Exception {
        for (int i = 0; i < 5; i++) {
            PaymentCardRequest request = new PaymentCardRequest();
            request.setUserId(testUser.getId());
            request.setNumber("123456789012345" + i);
            request.setHolder("CARD HOLDER " + i);
            request.setExpirationDate(LocalDateTime.now().plusYears(2));
        ...
        PaymentCardRequest sixthRequest = new PaymentCardRequest();
        sixthRequest.setUserId(testUser.getId());
        sixthRequest.setNumber("9999999999999999");
        sixthRequest.setHolder("SIXTH CARD");
        sixthRequest.setExpirationDate(LocalDateTime.now().plusYears(2));
    ...
    @Test
    void shouldNotCreateCardWhenUserNotFound() throws Exception {
        PaymentCardRequest request = new PaymentCardRequest();
